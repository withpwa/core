...
import { PWAsFile, FaviconHtmlElement, PWAsImage } from "../index";
...
const ICONS_OPTIONS: NamedIconOptions[] = [
  { name: "icon-36x36.png", ...transparentIcon(36) },
  { name: "icon-48x48.png", ...transparentIcon(48) },
  { name: "icon-72x72.png", ...transparentIcon(72) },
  { name: "icon-96x96.png", ...transparentIcon(96) },
  { name: "icon-144x144.png", ...transparentIcon(144) },
  { name: "icon-192x192.png", ...transparentIcon(192) },
  { name: "icon-256x256.png", ...transparentIcon(256) },
  { name: "icon-384x384.png", ...transparentIcon(384) },
  { name: "icon-512x512.png", ...transparentIcon(512) },
];

const ICONS_OPTIONS_MASKABLE: NamedIconOptions[] = [
  { name: "icon-maskable-36x36.png", ...maskable(transparentIcon(36)) },
  { name: "icon-maskable-48x48.png", ...maskable(transparentIcon(48)) },
  { name: "icon-maskable-72x72.png", ...maskable(transparentIcon(72)) },
  { name: "icon-maskable-96x96.png", ...maskable(transparentIcon(96)) },
  { name: "icon-maskable-144x144.png", ...maskable(transparentIcon(144)) },
  { name: "icon-maskable-192x192.png", ...maskable(transparentIcon(192)) },
  { name: "icon-maskable-256x256.png", ...maskable(transparentIcon(256)) },
  { name: "icon-maskable-384x384.png", ...maskable(transparentIcon(384)) },
  { name: "icon-maskable-512x512.png", ...maskable(transparentIcon(512)) },
];
...
export class PWAsPlatform extends Platform {
  constructor(options: PWAsOptions) {
    super(
      options,
      uniformIconOptions(options, options.icons.pwaIcon, ICONS_OPTIONS),
    );
  }

  override async createImages(sourceset: SourceImage[]): Promise<PWAsImage[]> {
    ...
    if (
      Array.isArray(this.options.screenshots) &&
      this.options.screenshots.length > 0
    ) {
  ...
  override async createFiles(): Promise<PWAsFile[]> {
  ...
  override async createHtml(): Promise<FaviconHtmlElement[]> {
    ...
    const themeMetaTags =
      Array.isArray(this.options.themes) && this.options.themes.length > 1
        ...
        : [
            `<meta name="theme-color" content="${this.options?.themes?.[0] || this.options?.background}">`,
          ];

    const appNameMetaTag = this.options.name
      ? `<meta name="application-name" content="${escapeHtml(this.options.name)}">`
  ...
  private async shortcutIcons(): Promise<PWAsImage[]> {
  ...
  private async screenshots(): Promise<PWAsImage[]> {
  ...
  private manifestFileName(): string {
    return (
      this.options.files?.pwaIcon?.manifestFileName ?? "manifest.webmanifest"
    );
  }

  private async manifest(): Promise<PWAsFile> {
    const { options } = this;
    ...
    const properties: Record<string, unknown> = {
      name: options.name,
      name_localized: options.manifest?.name_localized,
      short_name: options.manifest.short_name ?? options.name,
      short_name_localized: options.manifest.short_name_localized,
  ...
  private manifestShortcuts(basePath: string): Shortcut[] {
    return this.options.shortcuts
      .map((shortcut, index) => {
        ...
        return {
          name: shortcut.name,
          name_localized: shortcut.name_localized,
          short_name: shortcut.short_name || shortcut.name, // fallback to name
          short_name_localized: shortcut.short_name_localized,
          description: shortcut.description, // optional
          description_localized: shortcut.description_localized,
          url: shortcut.url,
          icons: shortcut.icon
      ...
      this.options.screenshots.map(async (screenshot, index) => {
        ...
        try {
          ...
          const screenshotImageSrc = this.cacheBusting(
            relativeTo(
              basePath,
              `${prefix}screenshots${index + 1}-${finalSizes}.${type?.split("/")[1] ?? "webp"}`,
            ),
          );
