...
import { PWAsFile, FaviconHtmlElement } from "../../index";
import { PWAsOptions, IconSize, NamedIconOptions } from "../../config/defaults";
...
export class WindowsPlatform extends Platform {
  constructor(options: PWAsOptions) {
    super(
      options,
      uniformIconOptions(options, options.icons.windowsTile, ICONS_OPTIONS),
    );
  ...
  private manifestFileName(): string {
    return (
      this.options.files?.windowsTile?.manifestFileName ?? "browserconfig.xml"
    );
  }

  private browserConfig(): PWAsFile {
